...
interface KrakenClosedTradeResponse {
    message: string;
    privateResponse: {
        closed: {
            [tradeId: string]: {
...
describe("API Tests", () => {
    ...
    it("should successfully call the private endpoint", async () => {
        ...
        expect(data.privateResponse.privateResponse.count).toBeDefined();
        expect(data.privateResponse.privateResponse.count).toBe(1);
    });
    ...
    it("should handle details of a single closed trade in privateResponse", async () => {
        ...
        expect(data.privateResponse.privateResponse.closed).toBeDefined();
        expect(
            data.privateResponse.privateResponse.closed["OCOIRM-3AVHM-7B2YLW"]
        ).toBeDefined();
        // Check specific properties of the closed trade
        const tradeDetails =
            data.privateResponse.privateResponse.closed["OCOIRM-3AVHM-7B2YLW"];
